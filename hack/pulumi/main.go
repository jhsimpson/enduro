...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		clusterName := "enduro-sdps-" + ctx.Stack()
		k8sCluster, err := digitalocean.NewKubernetesCluster(ctx, "k8s-cluster",
			&digitalocean.KubernetesClusterArgs{
				...
				NodePool: &digitalocean.KubernetesClusterNodePoolArgs{
					Name:      pulumi.String(clusterName + "-pool"),
		...
		for _, endpoint := range endpoints {
			host := endpoint.Name + "." + ctx.Stack() + ".sdps.artefactual.com"
		...
		for _, endpoint := range endpoints {
			dnsResource, err := route53.NewRecord(ctx, endpoint.Name+"-dns",
				&route53.RecordArgs{
					ZoneId: cfg.RequireSecret("route53ZoneId"),
					Name:   pulumi.String(endpoint.Name + "." + ctx.Stack() + ".sdps.artefactual.com"),
